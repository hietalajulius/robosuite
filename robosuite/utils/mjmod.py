...
class DynamicsModder(BaseModder):
    ...
    def __init__(
        ...
        self.body_names = list(
            self.sim.model.body_names) if body_names is None else body_names
        self.geom_names = list(
            self.sim.model.geom_names) if geom_names is None else geom_names
        self.joint_names = list(
            self.sim.model.joint_names) if joint_names is None else joint_names
    ...
    def save_defaults(self):
        ...
        for joint_name in self.sim.model.joint_names:
            ...
            dof_idx = [i for i, v in enumerate(
                self.sim.model.dof_jntid) if v == joint_id]
    ...
    def mod_frictionloss(self, name, val):
        ...
        if self.sim.model.jnt_type[jnt_id] != 0:
            dof_idx = [i for i, v in enumerate(
                self.sim.model.dof_jntid) if v == jnt_id]
